...
//        ArgumentCaptor<BalanceCallback> argument = ArgumentCaptor.forClass(BalanceCallback.class);
//        verify(kinManagerMock).getCurrentBalance(argument.capture());
//        argument.getValue().onBalanceError(anyString());
//        assertNotEquals(presenter.getBalance(), balance);
//        //TODO not working?????
//       // verify(mockView, never()).updateBalance(anyInt());
//    }
    ...
    public void onNextTest() {
        presenter.onNext();
        ArgumentCaptor<SendingKinCallback> argumentCaptor = ArgumentCaptor.forClass(SendingKinCallback.class);
        verify(kinManagerMock, never()).sendKin(anyString(), anyInt(), argumentCaptor.capture());
    }
    ...
    public void onNextStartTransferTest() {
        presenter.setAmount(amount);
        presenter.setRecipientAddress(address);
        navigatorMock.setStep(Navigator.STEP_CONFIRM);
        when(navigatorMock.shouldStartTransfer()).thenReturn(true);
        presenter.onNext();
        ArgumentCaptor<SendingKinCallback> argumentCaptor = ArgumentCaptor.forClass(SendingKinCallback.class);
        //TODO
        //verify(kinManagerMock).sendKin(address, amount, argumentCaptor.capture());
    }
     ...
     *
     *     @Override
     *     public void updateContact(@NonNull UUID id) {
     *         Log.d("#####", "##### updateContact contact " + contactName + "addres :" + recipientAddress + "#");
     *         contactsRepo.update(id, contactName, recipientAddress);
     *         reset();
     *     }
     *
     *     @Override
     *     public void saveNewContact() {
     *         Log.d("#####", "##### save contact " + contactName + "addres :" + recipientAddress + "#");
